// src/content/topics/algebra-ii/recurring-decimals/ExamplesSection.jsx
// Examples Section for Converting Recurring Decimals to Fractions
// Four tabs: Single digit, Two digit, Three digit, Non-recurring prefix
...
const ExamplesSection = ({ currentTopic, currentLessonId }) => {
  ...
  const generators = [
    ...
    recurringDecimalsGenerators.generateThreeDigitRecurring,
    recurringDecimalsGenerators.generateNonRecurringPrefix
  ];
            ...
            <div className="bg-orange-50 rounded-lg p-8 border-l-4 border-orange-400 mb-6">
              <div className="text-center">
                <div className="text-2xl">
                  <MathDisplay math={currentExample?.questionText?.replace(/^\$/, '').replace(/\$$/, '').replace(/\$/g, '')} displayMode={true} />
                </div>
              </div>
            </div>
            {showAnswers && currentExample?.solution && (
              <div className="space-y-4">
                ...
                <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                  <h5 className="font-medium text-purple-800 mb-2">Key Pattern</h5>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-center">
                    <div><MathDisplay math="0.\\dot{a} = \\frac{a}{9}" /><p className="text-purple-600 mt-1">1 digit</p></div>
                    <div><MathDisplay math="0.\\dot{a}\\dot{b} = \\frac{ab}{99}" /><p className="text-purple-600 mt-1">2 digits</p></div>
                    <div><MathDisplay math="0.\\dot{a}b\\dot{c} = \\frac{abc}{999}" /><p className="text-purple-600 mt-1">3 digits</p></div>
                  </div>
                </div>
              </div>
            )}
